...
class StochWindyGridWorldEnv(gym.Env):
            ...
            for w in self.w_range + self.range_random_wind:
                ...
                if s == self.goal_state:
                    ...
                    #                        print('wind=',wind)
                        ...
                        wind = 0
                    self.f[s, self.actions["U"], w] = self.dim2to1(
                        (max(i - 1 - wind, 0), j)
                    )
                    self.f[s, self.actions["R"], w] = self.dim2to1(
                        (
                            min(max(i - wind, 0), self.grid_height - 1),
                            min(j + 1, self.grid_width - 1),
                        )
                    )
                    self.f[s, self.actions["D"], w] = self.dim2to1(
                        (max(min(i + 1 - wind, self.grid_height - 1), 0), j)
                    )
                    self.f[s, self.actions["L"], w] = self.dim2to1(
                        (min(max(i - wind, 0), self.grid_height - 1), max(j - 1, 0))
                    )
    ...
    def _virtual_step_absorb(self, s, a, force_noise=None):
        ...
        noise = (
            ...
            else self.np_random.choice(self.w_range, 1, p=self.probabilities)[0]
        )
    ...
    def simulate_sample_path(self):
        ...
        sample_path = self.prng_simulator.choice(
            self.w_range, tau, p=self.probabilities
        )
        return sample_path
    ...
    def step_absorb(self, action, force_noise=None):
        ...
        self.observation, reward, terminated, wind, noise = self._virtual_step_absorb(
            self.observation, action, force_noise
        )
        self.realized_wind = wind
        return self.observation, reward, terminated, False, {"noise": noise}
    ...
    def _virtual_step_f(self, state, action, force_noise=None):
        ...
        if state == self.goal_state and destination == self.goal_state:
            ...
            reward = -1
            terminated = False
        return destination, reward, terminated, wind, noise
    ...
    def step(self, action, force_noise=None):
        ...
        action : 0 = Up, 1 = Right, 2 = Down, 3 = Left
        force_noise : int
            Force the noise to be a specific value. This is used for
            evaluating your agent's performance, and in general should not be
            used for learning.
        ...
        """
        assert self.action_space.contains(action)
        self.observation, reward, terminated, wind, noise = self._virtual_step_f(
            self.observation, action, force_noise
        )
        self.realized_wind = wind
        return self.observation, reward, terminated, False, {"noise": noise}
